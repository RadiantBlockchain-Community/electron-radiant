...
export DEFAULT_GIT_REPO=https://github.com/RadiantBlockchain-Community/electron-radiant
if [ -z "$GIT_REPO" ] ; then
...
if [ "${GIT_SUBMODULE_SKIP:-0}" -eq 0 ] ; then
    ...
    git $gitflags submodule update --init --jobs $WORKER_COUNT $GIT_SUBMODULE_FLAGS || fail "Failed to update git submodules"
fi
